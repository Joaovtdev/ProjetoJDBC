...
public class TestJDBC {
    ...
    public void testCadastrarProduto() {
        try {
            Main.cadastrarProduto(new Scanner(new ByteArrayInputStream("\nTest Product\n10.99\n".getBytes())));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void testCadastrarVenda() {
        try {
            Main.cadastrarVenda(new Scanner(new ByteArrayInputStream("\n1\n1\n".getBytes())));
        } catch (Exception e) {
            Assert.assertFalse(Boolean.parseBoolean("Falha ao cadastrar venda: " + e.getMessage()));
        }
    }

    public void testeAlterarVenda(){
        try{
            Main.atualizarVenda(new Scanner(new ByteArrayInputStream("3\n3\n3\n".getBytes())));
        } catch (Exception e){
